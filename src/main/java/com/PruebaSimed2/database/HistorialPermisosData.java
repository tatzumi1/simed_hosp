...
public class HistorialPermisosData {
    private static final String OTORGAR_PERMISO_INTER = "INSERT INTO tb_historial_permisos (id_nota, tipo_nota, folio_paciente, medico_autor, " +
    ...
    public void otorgarPermisoInter(Connection connection, InsertarPermisoDTO dto) {
        try (PreparedStatement statement = connection.prepareStatement(OTORGAR_PERMISO_INTER)) {
            configurarStatement(dto, statement);
            log.info("Permiso de notas otorgado correctamente para la interconsulta: {}", dto.getId());
        } catch (SQLException e) {
            log.error("Error al otorgar permiso de interconsulta a un paciente", e);
    ...
    private void configurarStatement(InsertarPermisoDTO dto, PreparedStatement statement) throws SQLException {
